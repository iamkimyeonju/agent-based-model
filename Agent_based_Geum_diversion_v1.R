# ------------------------------------------------------------------------------
# Agent-based_Geum_diversion
# Last edit: UConn computer 2026/01/03-17:03
# ------------------------------------------------------------------------------
...
# Water balance -----------------------------------------------------------
  ...
  # for syntax end : day
    ...
    if ((i %in% breaker[["ABM"]]) && (any(breaker$month[breaker$ABM == i] %in% c(5, 6, 7, 8))) && ABM == TRUE) { #i=61 
      ...
      diversion1_active <- rep(FALSE, opt)
      diversion2_active <- rep(FALSE, opt)
      diversion3_active <- rep(FALSE, opt)
      diversion4_active <- rep(FALSE, opt)
      diversion5_active <- rep(FALSE, opt)
      diversion6_active <- rep(FALSE, opt)
      diversion7_active <- rep(FALSE, opt)
      ...
      ABM_Geum <- function(top3_pos, agent) { #k=3 # 1. top3 위치에서 각 셀까지의 맨하탄 거리 계산 (3개 지점에 대해) 
        ...
        distance_list <- lapply(1:3, function(k) { #k=2 
          ...
          if (is.na(row) & is.na(col)){ invisible(NULL)} 
            ...
            invisible(NULL) } 
              ...
              distance_matrix[7,1] <- 1} 
          else if (row == 9 & col == 2){ # Agr 9 
            invisible(NULL)} 
          else if (row == 11 & col == 2){ # Agr 11 
            distance_matrix[12,2] <- 1 
            distance_matrix[11,1] <- 1 } 
          else if (row ==11 & col == 1){ # Intake 11 
            invisible(NULL) } 
          else if(row == 12 & col == 1){ # Intake 12 
            invisible(NULL) }
          else if (row ==13 & col == 2){ # Agr 13 
            distance_matrix[14,2] <- 1  }
          else{ 
        ...
        return(agent_prob) } # ABM_Geum end

    
      agent_prob <- ABM_Geum(top3_pos, agent)

      # 1) 안전한 인덱스 + 업데이트 구간 계산
      idx <- which(breaker$ABM == i)              # 월말 행 인덱스 (길이 1)
      if (length(idx) == 1 && !is.na(idx)) {
        decision_start <- i - breaker$days[idx] + 1
        decision_end   <- i
      ...
      # 2) agent_prob -> 활성화 확률 변환 함수(스트레스 기반)
      #    ar = agent_prob[row, col] (1이면 무스트레스, 0.8/0.9면 스트레스)
      prob_from_agent <- function(ar, p0 = 0.15, gamma = 0.75, beta = 1.0) {
        if (is.na(ar)) return(0)                  # 정보 없으면 0
        stress <- max(0, min(1, 1 - ar))         # 0(양호) ~ 1(최악)
        p <- p0 + gamma * (stress^beta)           # 기본확률 + 스트레스 가중
        p <- min(1, max(0, p))                    # [0,1]로 클램프
        return(p)
      }
      
      # 3) (row,col) -> diversion ID 매핑 헬퍼
      diversion_id_of <- function(r, c) {
        ids <- c()
        
        if (r == 5  && c == 2) ids <- c(ids, 1, 5)  # Agr5
        if (r == 7  && c == 2) ids <- c(ids, 2, 6)  # Agr7 → diversion 2, 6 동시에
        if (r == 11 && c == 2) ids <- c(ids, 3, 7)  # Agr10 → diversion 3, 7 동시에
        if (r == 12 && c == 2) ids <- c(ids, 4)  # Agr12
      ...
      activate_diversion_window <- function(k) {
        if (is.na(k)) return(invisible(NULL))
        if (k == 1) diversion1_active[update_start:update_end] <<- TRUE
        if (k == 2) diversion2_active[update_start:update_end] <<- TRUE
        if (k == 3) diversion3_active[update_start:update_end] <<- TRUE
        if (k == 4) diversion4_active[update_start:update_end] <<- TRUE
        if (k == 5) diversion5_active[update_start:update_end] <<- TRUE
        if (k == 6) diversion6_active[update_start:update_end] <<- TRUE
        if (k == 7) diversion7_active[update_start:update_end] <<- TRUE
      }
      
      # 5) top3 후보 각각에 대해 확률적(베르누이)로 도수로 on 결정
      #    (top3_pos는 3x2 matrix, 각 행이 (row, col))
      for (k in 1:nrow(top3_pos)) {
        r <- top3_pos[k, 1]; c <- top3_pos[k, 2]
        if (is.na(r) || is.na(c)) next
        ar <- agent_prob[r, c]
        p_act <- prob_from_agent(ar)
        turn_on <- as.logical(rbinom(1, 1, p_act))
        
        if (turn_on) {
          did_vec <- diversion_id_of(r, c)
          if (!is.na(did_vec[1])) {
            for (did in did_vec) {
              activate_diversion_window(did)   # 여러 diversion 동시 on 가능
            }
          }
        }
      }
