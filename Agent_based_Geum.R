...
Agent_Geum_Network <- function(supply_scenario = 1, demand_scenario = 1,  opt_year = 10, ABM=TRUE){
...
# Water balance -----------------------------------------------------------
  ...
  for(i in 1:c(365*opt_year)){ #Start from October
    ...
    #for(i in 1:274){ #Start from October
    ### Intake 1
    ...
    Node_C <- Node_B - Intake5 + (Intake5 * alpha_Intake) - Agr6 + (Agr6 * alpha_Agr) + 
              inflow_3007[i,] - Intake6 + (Intake6 * alpha_Intake) - Agr7 + (Agr7 * alpha_Agr)
