...
Agent_Geum_Network_Diversion <- function(supply_scenario = 1, demand_scenario = 1,  opt_year = 10, ABM=TRUE,
                                          p_vec = c(0.9, 0.6, 0.3, 0.1)){
  # p_vec: activation probabilities for Manhattan distance d = 1, 2, 3, >=4
  # Default = Bounded Rational scenario (0.9, 0.6, 0.3, 0.1)
...
# General setting ---------------------------------------------------------
  ...
  # Fix: remove Feb 29 (leap year days) to match the 365-day-per-year assumption used in
  # breaker, demand_dam, demand_intake, and demand_agr arrays
  runoff_array <- runoff_array %>% filter(!(month(Time) == 2 & day(Time) == 29))
  ...
  inflow_3011 <- inflow_3011 - inflow_301103
  ...
  breaker["first_row"] <- NA
  breaker["first_col"] <- NA
  breaker["second_row"] <- NA
  breaker["second_col"] <- NA
  breaker["third_row"] <- NA
  breaker["third_col"] <- NA
  ...
  diversion2_active <- rep(FALSE, opt) # Node C --> Agr7
  diversion3_active <- rep(FALSE, opt) # Node E --> Agr10
  diversion4_active <- rep(FALSE, opt) # Node F --> Agr12
  diversion5_active <- rep(FALSE, opt) # Intake 4 --> Agr5
  diversion6_active <- rep(FALSE, opt) # Intake 6 --> Agr7
  diversion7_active <- rep(FALSE, opt) # Intake 11 --> Agr10
...
# Water balance -----------------------------------------------------------
  ...
  for(i in 1:c(365*opt_year)){ #Start from October
    ...
    ### diversion 5
    if (diversion5_active[i]){
      help5_tributary <- Intake4 * 0.2
    }else{
      help5_tributary <- 0
    }
    df_intake[i,4] <- Intake4 - help5_tributary
    
    ### Agr 5
    if(inflow_3005[i,] - Intake4 + ((Intake4- help5_tributary)* alpha_Intake) >= demand_agr[i,5]- help5_tributary){
      Agr5 <- demand_agr[i,5] - help5_tributary
    }else if(inflow_3005[i,] - Intake4 + (Intake4- help5_tributary) * alpha_Intake > 0){
      Agr5 <- inflow_3005[i,] - Intake4 + Intake4 * alpha_Intake - help5_tributary
    }else{
      Agr5 <- 0
    }
    ...
    ### diversion 1
    want5 <-  demand_agr[i,5] - Agr5 - help5_tributary
    
    if (diversion1_active[i] && Node_B_dash >= want5){
      help5 <- want5
    }else{
      help5 <- 0
    }
    ...
    ### diversion 6
    if (diversion6_active[i]){
      help7_tributary <- Intake6 * 0.2
    }else{
      help7_tributary <- 0
    }
    
    df_intake[i,6] <- Intake6 - help7_tributary
    
    ### Agr 7
    if(inflow_3007[i,] - Intake6 + ((Intake6 - help7_tributary) * alpha_Intake) >= demand_agr[i,7] - help7_tributary){
      Agr7 <- demand_agr[i,7] - help7_tributary
    }else if (inflow_3007[i,] - Intake6 + ((Intake6 - help7_tributary) * alpha_Intake)  > 0){
      Agr7 <- inflow_3007[i,] - Intake6 + ((Intake6 - help7_tributary) * alpha_Intake) - help7_tributary
    }else{
      Agr7 <- 0
    }
    ...
    ### New diversion
    want7_main <-  demand_agr[i,7] - Agr7 - help7_tributary
    
    if (diversion2_active[i] && Node_C_dash >= want7_main){
      help7_main <- want7_main
    }else{
      help7_main <- 0
    }
    ...
    Node_C <- Node_B - Intake5 + (Intake5 * alpha_Intake) - Agr6 + (Agr6 * alpha_Agr) + 
              inflow_3007[i,] - Intake6 + ((Intake6 - help7_tributary) * alpha_Intake) - 
              Agr7 + ((Agr7 + help7_tributary + help7_main) * alpha_Agr) - help7_main 
    ...
    if(Dam2_Stor > dam2_Stor_Max){ #Flood
      Dam2_Spillway <- Dam2_Stor + inflow_3008[i,] + Node_C - Intake7 + (Intake7 * alpha_Intake) - dam2_Stor_Max
    ...
    ### Agr 8
    # available flow from Daecheong dam downstream, after Intake8/9/10
    # Fix: replaced 'Node_D > 0' with the correct available-flow expression (Node_D is computed
    # later and would use stale value from the previous iteration)
    if((R2_Agr * alpha_Agr) + (R2_Ind * alpha_Ind) + (R2_Env * alpha_Env) +
       Dam2_Spillway - Intake8 + (Intake8 * alpha_Intake) - Intake9 - Intake10 + (Intake10 * alpha_Intake) >= demand_agr[i,8] - R2_Agr){
      Agr8 <- demand_agr[i,8] - R2_Agr
    }else if ((R2_Agr * alpha_Agr) + (R2_Ind * alpha_Ind) + (R2_Env * alpha_Env) +
              Dam2_Spillway - Intake8 + (Intake8 * alpha_Intake) - Intake9 - Intake10 + (Intake10 * alpha_Intake) > 0){
      Agr8 <- (R2_Agr * alpha_Agr) + (R2_Ind * alpha_Ind) + (R2_Env * alpha_Env) +
        Dam2_Spillway - Intake8 + (Intake8 * alpha_Intake) - Intake9 - Intake10 + (Intake10 * alpha_Intake)
    }else{
      Agr8 <- 0
    }
    df_agr[i,8] <- Agr8 + R2_Agr
    ...
    ### Node D
    # Fix: added Dam2_Spillway for consistency (Node_A includes Dam1_Spillway, Node_E includes Dam4_Spillway;
    # Agr8's available-flow already accounts for Dam2_Spillway)
    Node_D <- (R2_Agr * alpha_Agr) + (R2_Ind * alpha_Ind) + (R2_Env * alpha_Env) + Dam2_Spillway -
              Intake8 + (Intake8 * alpha_Intake) - Intake9 - Intake10 + (Intake10 * alpha_Intake) - Agr8 + (Agr8*alpha_Agr) +
              inflow_3009[i,] - Agr9 + (Agr9 * alpha_Agr) + inflow_3010[i,]
    ...
    ### diversion 7
    if (diversion7_active[i]){
      help11_tributary <- Intake11 * 0.2
    }else{
      help11_tributary <- 0
    }
    df_intake[i,11] <- Intake11 - help11_tributary
    
    ### Agr 11
    if(R4_Agr * alpha_Agr + Dam4_Spillway + inflow_3011[i,] + (Intake9 * alpha_Intake) - Intake11 + ((Intake11-help11_tributary) * alpha_Intake) >= demand_agr[i,11]- R4_Agr - help11_tributary){
      Agr11 <- demand_agr[i,11] - R4_Agr - help11_tributary
    }else if (R4_Agr * alpha_Agr + Dam4_Spillway + inflow_3011[i,] + (Intake9 * alpha_Intake)- Intake11 + ((Intake11-help11_tributary) * alpha_Intake) > 0){
      Agr11 <- R4_Agr * alpha_Agr + Dam4_Spillway + inflow_3011[i,] + (Intake9 * alpha_Intake)- Intake11 + ((Intake11-help11_tributary) * alpha_Intake) - help11_tributary
    }else{
      Agr11 <- 0
    }
    ...
    # 금강공주 3012
    ### Dashed Node E 
    Node_E_dash <- Node_D - Agr10 + (Agr10 * alpha_Agr) - Power3 + (Power3 * alpha_Pow) + inflow_3012[i,]
    
    ### diversion 3
    want11_main <- demand_agr[i,11] - Agr11 - help11_tributary - R4_Agr
    
    if (diversion3_active[i] && Node_E_dash >= want11_main){
      help11_main <- want11_main
    }else{
      help11_main <- 0
    }
    ...
    ### Node E
    Node_E <- Node_D - Agr10 + (Agr10 * alpha_Agr) - Power3 + (Power3 * alpha_Pow) + inflow_3012[i,] +
              (R4_Agr * alpha_Agr) + Dam4_Spillway - Intake11 + ((Intake11- help11_tributary) * alpha_Intake) + 
              inflow_3011[i,] + inflow_301103[i,] + (Intake9 * alpha_Intake) - Agr11 + 
              ((Agr11 + help11_tributary+ help11_main) * alpha_Agr) - help11_main
    ...
    ### Agr 13
    if(Dam5 * alpha_Agr + Dam5_Spillway >= demand_agr[i,13] - Dam5){
      Agr13 <- demand_agr[i,13]  - Dam5
    }else if (Dam5 * alpha_Agr + Dam5_Spillway > 0){
      Agr13 <- Dam5 * alpha_Agr + Dam5_Spillway
    }else{
      Agr13 <- 0
    }
    
    ### Dashed Node F
    Node_F_dash <- Node_E - Agr12 + (Agr12 * alpha_Agr) 
    
    ### diversion 4
    want13 <- demand_agr[i,13] - Agr13 - Dam5
    
    if (diversion4_active[i] && Node_F_dash >= want13){
      help13 <- want13
    }else{
      help13 <- 0
    }
    ...
    df_want[i,4] <- want13
    df_help[i,4] <- help13
    df_agr[i,13] <- Agr13 + help13 + Dam5
    
    # 금강하구언 3014
    ## Node F
    Node_F <- Node_E - Agr12 + (Agr12 * alpha_Agr) + R5_Agr * alpha_Agr - Agr13 + (Agr13 *alpha_Agr) +Dam5_Spillway - help13 + inflow_3014[i,]
  ...
  # for syntax end : day
    ...
    if ((i %in% breaker[["ABM"]]) && (any(breaker$month[breaker$ABM == i] %in% c(5, 6, 7, 8))) && ABM == TRUE) { # e.g., i=243 (end of May, year 1); triggered at end of May/Jun/Jul/Aug each year
      decision_start <- i - breaker[which(i == breaker[["ABM"]]),"days"] +1 
      decision_end <- i 
      Shell <- data.frame(matrix(NA, ncol=3, nrow=14)) 
      agent <- Shell 
      ...
      donor_map <- list(
        "div1" = list(row = 5, col = 1), 
        "div2" = list(row = 7, col = 1),
        "div3" = list(row = 12, col = 2),
        "div4" = list(row = 14, col = 2),
        "div5" = list(row = 4, col = 1),
        "div6" = list(row = 6, col = 1),
        "div7" = list(row= 11,col =1)
      )
      
      recipient_map <- list(
        "div1" = list(row = 5, col = 2),
        "div2" = list(row = 7, col = 2),
        "div3" = list(row = 11, col = 2),
        "div4" = list(row = 13, col = 2),
        "div5" = list(row = 5, col = 2),
        "div6" = list(row = 7, col = 2),
        "div7" = list(row = 11, col = 2)
      )
      
      p_from_distance <- function(d) {
        if (is.na(d)) return(0)
        if (d == 1) return(p_vec[1])
        if (d == 2) return(p_vec[2])
        if (d == 3) return(p_vec[3])
        return(p_vec[4])  # d >= 4
      }
      
      bernoulli_on <- function(p) {
        runif(1) < p
      }
      
      manhattan <- function(r1, c1, r2, c2) abs(r1 - r2) + abs(c1 - c2)
      
      diversions <- function(top3_pos, donor_map, recipient_map) {
        
        diversion_ids <- sort(unique(names(donor_map)))
        diversion_active_next <- setNames(rep(FALSE, length(diversion_ids)), diversion_ids)
  
        for (div_id in diversion_ids) {
          
          target <- recipient_map[[div_id]]  # list(row=?, col=?)
          if (is.null(target)) next
          
          is_target_in_top3 <- any(apply(top3_pos, 1, function(rc) all(rc == c(target$row, target$col))))
          if (!is_target_in_top3) next
          
          donor <- donor_map[[div_id]]      # list(row=?, col=?)
          
          d <- manhattan(donor$row, donor$col, target$row, target$col)
          p_on <- p_from_distance(d)
          
          diversion_active_next[[div_id]] <- bernoulli_on(p_on)
        }
        
        return(diversion_active_next)
      }

      div_next <- diversions(top3_pos, donor_map, recipient_map )
      
      # next month window
      # Fix: use breaker$days[idx+1] (next month's days) not breaker$days[idx]-1 (current month's days - 1).
      # Paper: "the diversion state determined at end of month m governs tunnel operation throughout month m+1"
      update_start <- decision_end + 1
      update_end   <- decision_end + breaker$days[idx + 1]
      
      # 4) 활성화 플래그 벡터(한 달 범위) 준비 (미리 전역에서 FALSE로 초기화해둔 상태 권장)
      diversion1_active[update_start:update_end] <- div_next[["div1"]]
      diversion2_active[update_start:update_end] <- div_next[["div2"]]
      diversion3_active[update_start:update_end] <- div_next[["div3"]]
      diversion4_active[update_start:update_end] <- div_next[["div4"]]
      diversion5_active[update_start:update_end] <- div_next[["div5"]]
      diversion6_active[update_start:update_end] <- div_next[["div6"]]
      diversion7_active[update_start:update_end] <- div_next[["div7"]]

      
      # (선택) 기록: 어디서 켰는지 breaker에 남김
      breaker$first_row[idx]  <- top3_pos[1, 1]
      breaker$first_col[idx]  <- top3_pos[1, 2]
      breaker$second_row[idx] <- top3_pos[2, 1]
      breaker$second_col[idx] <- top3_pos[2, 2]
      breaker$third_row[idx]  <- top3_pos[3, 1]
      breaker$third_col[idx]  <- top3_pos[3, 2]
      
    print(paste("[ABM] ABM run completed — Day",i))
  } # ABM end
  } # water allocation end
# Result checking -------------------------------------------------------------
...
# Result ---------------------------------------------------------------------
  ...
  gg_agr <- table_agr %>%
    ...
    ylab("Sum of Reliability")+
    ggtitle("Reliability of Agricultural Water Use")
  ...
  irrigation_days <- rowSums(demand_agr > 0) > 0
  gini_intake <- Gini(colSums(df_intake, na.rm=TRUE)/colSums(demand_intake, na.rm=TRUE))
  gini_power <- Gini(colSums(df_power, na.rm=TRUE)/colSums(demand_power, na.rm=TRUE))
  gini_agr <- Gini(colSums(df_agr, na.rm=TRUE) / colSums(demand_agr, na.rm=TRUE))
 
  gini_total <- Gini(c( colSums(df_intake[irrigation_days,], na.rm=TRUE)/colSums(demand_intake[irrigation_days,], na.rm=TRUE), 
                        colSums(df_power[irrigation_days,], na.rm=TRUE)/colSums(demand_power[irrigation_days,], na.rm=TRUE), 
                        colSums(df_agr[irrigation_days,], na.rm=TRUE)/colSums(demand_agr[irrigation_days,], na.rm=TRUE)), 
                     na.rm = TRUE)

  activation <- data.frame(diversion1_active, diversion2_active, diversion3_active,
                          diversion4_active, diversion5_active, diversion6_active, diversion7_active)
  ...
  Result_list <- list(df_intake, df_agr, df_power, demand_intake,demand_agr,demand_power, 
                      ...
                      profit_ratio_power,profit_ratio_agr,profit_ratio_intake, breaker, df_help, activation)
