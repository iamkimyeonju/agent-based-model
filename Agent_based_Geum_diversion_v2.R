...
# Water balance -----------------------------------------------------------
  ...
  for(i in 1:c(365*opt_year)){ #Start from October
    ...
    if (diversion7_active[i]){
      help10_tributary <- Intake11 * 0.2
    }else{
      help10_tributary <- 0
    }
    df_intake[i,11] <- Intake11 - help10_tributary
    
    ### Agr 10
    if(R4_Agr * alpha_Agr + Dam4_Spillway + inflow_3011[i,] + (Intake9 * alpha_Intake) - Intake11 + ((Intake11-help10_tributary) * alpha_Intake) >= demand_agr[i,10]- R4_Agr - help10_tributary){
      Agr10 <- demand_agr[i,10] - R4_Agr - help10_tributary
    }else if (R4_Agr * alpha_Agr + Dam4_Spillway + inflow_3011[i,] + (Intake9 * alpha_Intake)- Intake11 + ((Intake11-help10_tributary) * alpha_Intake) > 0){
      Agr10 <- R4_Agr * alpha_Agr + Dam4_Spillway + inflow_3011[i,] + (Intake9 * alpha_Intake)- Intake11 + ((Intake11-help10_tributary) * alpha_Intake) - help10_tributary
    }else{
      Agr10 <- 0
    }
    
    ### Dashed Node E 
    Node_E_dash <- Node_D - Agr9 + (Agr9 * alpha_Agr) - Power3 + (Power3 * alpha_Pow)
    
    ### diversion 3
    want10_main <- demand_agr[i,10] - Agr10 - help10_tributary - R4_Agr
    
    if (diversion3_active[i] && Node_E_dash >= want10_main){
      help10_main <- want10_main
    }else{
      help10_main <- 0
    }
    
    df_want[i,3] <- want10_main
    df_help[i,3] <- help10_main
    df_agr[i,10] <- Agr10 + help10_main + help10_tributary + R4_Agr
    
    ### Node E
    Node_E <- Node_D - Agr9 + (Agr9 * alpha_Agr) - Power3 + (Power3 * alpha_Pow) +
              (R4_Agr * alpha_Agr) + Dam4_Spillway - Intake11 + ((Intake11- help10_tributary) * alpha_Intake) + 
              inflow_3011[i,] + (Intake9 * alpha_Intake) - Agr10 + ((Agr10+help10_tributary+help10_main) * alpha_Agr) - help10_main
    ...
    ### Agr 12
    if(Dam5 * alpha_Agr + Dam5_Spillway >= demand_agr[i,12] - Dam5){
      Agr12 <- demand_agr[i,12]  - Dam5
    }else if (Dam5 * alpha_Agr + Dam5_Spillway > 0){
      Agr12 <- Dam5 * alpha_Agr + Dam5_Spillway
    }else{
      Agr12 <- 0
    }
    
    ### Dashed Node F
    Node_F_dash <- Node_E - Agr11 + (Agr11 * alpha_Agr) 
    
    ### diversion 4
    want12 <- demand_agr[i,12] - Agr12 - Dam5
    
    if (diversion4_active[i] && Node_F_dash >= want12){
      help12 <- want12
    }else{
      help12 <- 0
    }
    ...
    df_want[i,4] <- want12
    df_help[i,4] <- help12
    df_agr[i,12] <- Agr12 + help12 + Dam5
    
    ## Node F
    Node_F <- Node_E - Agr11 + (Agr11 * alpha_Agr) + R5_Agr * alpha_Agr - Agr12 + (Agr12 *alpha_Agr) +Dam5_Spillway - help12
  ...
  # for syntax end : day
    ...
    if ((i %in% breaker[["ABM"]]) && (any(breaker$month[breaker$ABM == i] %in% c(5, 6, 7, 8))) && ABM == TRUE) { #i=61 
      ...
      top3_pos <- arrayInd(top3_idx, dim(agent)) 
        ...
        distance_list <- lapply(1:3, function(k) {
        ...
        min_distance <- matrix(Inf, nrow=nrow(agent), ncol=ncol(agent)) 
        
        for (n in 1:3) { 
          if (!all(is.na(distance_list[[n]]))) { 
            min_distance <- pmin(min_distance, distance_list[[n]], na.rm=TRUE) } } 
        min_distance[is.infinite(min_distance)] <- NA 
        
        agent_prob  <- matrix(0, nrow=nrow(agent), ncol=ncol(agent))
        for (g in 1:nrow(agent)) { 
          for (t in 1:ncol(agent)) { 
            d <- min_distance[g,t] 
            if(!is.na(d)) { 
              if(d == 1) {agent_prob[g,t] <- 0.9 } 
              else if(d == 2) { agent_prob[g,t] <-0.8} 
              else if(d == 3) { agent_prob[g,t] <- 0.7 }
              } } }  

        active <- matrix(FALSE,
                         nrow = nrow(agent_prob),
                         ncol = ncol(agent_prob))
        
        for (g in 1:nrow(agent_prob)) {
          for (t in 1:ncol(agent_prob)) {
            p <- agent_prob[g, t]
            if (!is.na(p) && p > 0) {
              active[g, t] <- runif(1) < p
            }
          }
        }
      ...
      diversion1_active[update_start:update_end] <- active[5,1]
      diversion2_active[update_start:update_end] <- active[7,1]
      diversion3_active[update_start:update_end] <- active[11,2]
      diversion4_active[update_start:update_end] <- active[12,2]
      diversion5_active[update_start:update_end] <- active[13,2]
      diversion6_active[update_start:update_end] <- active[4,1]
      diversion7_active[update_start:update_end] <- active[6,1]
